...
void main() {
  ...
  vec2 sC = uContain > 0.5 ? containUV(uvC, uResolution, uCurrentSize) : coverUV(uvC, uResolution, uCurrentSize);
  vec2 sN = uContain > 0.5 ? containUV(uvN, uResolution, uNextSize) : coverUV(uvN, uResolution, uNextSize);
  ...
  if (uContain > 0.5) {
    colC = mix(uOverlay, colC, imageBounds(sC));
    colN = mix(uOverlay, colN, imageBounds(sN));
  }
...
class MorphEngine {
  ...
  constructor(container: HTMLElement, config: EngineConfig) {
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uContain: { value: opts.imageFit === 'contain' ? 1 : 0 }
      }
    });
  ...
  private syncOptions(): void {
    ...
    this.program.uniforms.uContain.value = opts.imageFit === 'contain' ? 1 : 0;
  }
  ...
  select(index: number): void {
    ...
    if (target === this.current || this.animating || this.dragging) return;
    const opts = this.getOptions();
    ...
    this.animating = true;
    const duration = this.reducedMotion ? Math.min(opts.duration, 0.4) : opts.duration;
    this.tween = gsap.fromTo(this.program.uniforms.uProgress, { value: 0 }, {
      value: 1,
      duration,
      ease: opts.ease,
      onComplete: () => this.commit(target),
...
export default function MorphSlider({
  ...
  optsRef.current = { imageFit, transition, duration, ease, intensity, scale, aberration, drift, overlayColor, loop };
  ...
  useEffect(() => {
    ...
    const engine = new MorphEngine(containerRef.current, {
      ...
      onIndexChange: (nextIndex) => {
        emittedIndexRef.current = nextIndex;
        setIndex(nextIndex);
  ...
  useEffect(() => {
    if (emittedIndexRef.current === startIndex) {
      emittedIndexRef.current = null;
      return;
    }
    engineRef.current?.select(startIndex);
    setIndex(startIndex);
  }, [startIndex]);
